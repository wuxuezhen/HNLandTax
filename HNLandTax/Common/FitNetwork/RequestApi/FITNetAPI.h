...
/**
 ...
 @param fitRequst 请求参数 注：需要配置 链接：path，请求方式：requestMethod, 解析方式：parseMethod ，条件参数：parameters ，解析类：responseModel（解析方式部位no时，必传字段）;
 @param success 成功回调
 @param failure 失败回调
 @return task
 */
+ (NSURLSessionDataTask *_Nonnull)FITRequst:(FITNetRequst *_Nonnull)fitRequst
