...
-(void)wz_evaluatePolicy:(void (^)(BOOL, NSString * _Nullable))replyBlock{
    ...
    if ([self wz_canEvaluatePolicy]) {
        NSString *localizedReasonKey = @"请验证已有指纹";
                     ...
                     localizedReason:localizedReasonKey
                               reply:^(BOOL success, NSError * _Nullable error) {
                                   ...
                                   dispatch_async(dispatch_get_main_queue(), ^{
                                       replyBlock ? replyBlock(success, message) : nil;;
                                   });
                               }];
...
-(BOOL)wz_canEvaluatePolicy{
    ...
    if (NSFoundationVersionNumber < NSFoundationVersionNumber_iOS_8_0) {
        ...
        NSError *error = nil;
        return [self.context canEvaluatePolicy:LAPolicyDeviceOwnerAuthentication error:&error];
    }
}
