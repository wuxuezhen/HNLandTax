...
- (void)getNextPage {
    ...
    self.currentDataTask = [FITAPI GET:self.path params:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        __strong typeof(self) strongSelf = wself;
        if (!strongSelf) {
            return ;
        }
        strongSelf.loading = NO;
        NSArray *results = [FITAPI parseIntoArrayOfClass:self.klass fromArray:responseObject[self.key ? : @"result"]];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            strongSelf.NetworkingErrorHandler ? strongSelf.NetworkingErrorHandler(error, [error.userInfo objectForKey:FITMETAERRMSGKEY]) : nil;
            [strongSelf refreshCompletion];
        });
